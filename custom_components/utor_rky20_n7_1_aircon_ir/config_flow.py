"""Config flow for Go-On UTOR-RKY20-N7-1 Aircon IR (remote + infrared emitter)."""
...
class AirconIrConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle config flow — new infrared emitter preferred, remote for legacy."""
    ...
    async def async_step_user(
        ...
        # try to list infrared emitters for preferred path (optional dependency)
        try:
            from homeassistant.components.infrared import async_get_emitters

            infrared_emitters = async_get_emitters(self.hass)
        except Exception:
            infrared_emitters = []
        ...
        if user_input is not None:
            ...
            elif not user_input.get(CONF_REMOTE_ENTITY_ID) and not user_input.get(
                CONF_INFRARED_ENTITY_ID
            ):
                errors["base"] = "need_one_entity"
            else:
                unique = user_input.get(CONF_INFRARED_ENTITY_ID) or user_input.get(
                    CONF_REMOTE_ENTITY_ID
                )
        ...
        # selectors: remote (legacy) optional, infrared (new) optional + recommended
        schema_dict: dict[Any, Any] = {
            vol.Required(CONF_NAME, default=DEFAULT_NAME): str,
            vol.Optional(CONF_REMOTE_ENTITY_ID): selector.EntitySelector(
                selector.EntitySelectorConfig(domain="remote")
            ),
            vol.Optional(CONF_INFRARED_ENTITY_ID): selector.EntitySelector(
            ...
            vol.Required(CONF_DEFAULT_SWING_MODE, default=DEFAULT_SWING_MODE): vol.In(
                SWING_MODES
            ),
        ...
        # If infrared integration provides no emitters, hint remains but validation will catch
